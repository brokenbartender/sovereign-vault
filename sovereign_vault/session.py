"""
sovereign_vault.session — VaultSession class and session registry.
...
from .patterns import REGEX_PATTERNS, VAULT_TOKEN_RE, GLINER_TYPES
...
class VaultSession:
    ...
    def __init__(
        ...
        self._abstract_text: Optional[str] = None
    ...
    def _layer_regex(self, text: str) -> str:
        ...
        for label, pattern in REGEX_PATTERNS.items():
            for m in re.finditer(pattern, text):
                if VAULT_TOKEN_RE.search(m.group()):
                    continue
                spans.append(Span(
    ...
    def _layer_gliner(self, text: str) -> tuple[str, int]:
        ...
        try:
            raw = self._gliner.predict_entities(
                text, GLINER_TYPES, threshold=self.gliner_threshold
            )
    ...
    def tokenize(self, raw_text: str) -> str:
        ...
        if VAULT_TOKEN_RE.search(raw_text):
    ...
    def diff(self) -> str:
        """
        Returns a human-readable diff showing what was tokenized.

        Call after tokenize() to see exactly what PII was detected,
        which layer found it, and what token replaced it.

        Example output:
            DETECTED 3 entities:
              [regex]  SSN        123-45-6789       → [[SSN_A1B2C3D4_e5f6a7]]      confidence=1.00
              [regex]  EMAIL      john@firm.com     → [[EMAIL_F8G9H0I1_j2k3l4]]    confidence=1.00
              [gliner] PERSON     John Doe          → [[PERSON_M5N6O7P8_q9r0s1]]   confidence=0.92
        """
        self._assert_alive()
        if not self._store:
            return "No entities detected."

        lines = [f"DETECTED {len(self._store)} entities:\n"]
        for key, entry in self._store.items():
            source = entry.source_layer.ljust(6)
            label = entry.label.ljust(14)
            value = entry.real_value[:20].ljust(20)
            conf = f"confidence={entry.confidence:.2f}"
            lines.append(f"  [{source}] {label} {value} -> {key}  {conf}")

        if self._original_text and self._abstract_text:
            lines.append(f"\nORIGINAL ({len(self._original_text)} chars):")
            lines.append(f"  {self._original_text[:200]}{'...' if len(self._original_text) > 200 else ''}")
            lines.append(f"\nTOKENIZED ({len(self._abstract_text)} chars):")
            lines.append(f"  {self._abstract_text[:200]}{'...' if len(self._abstract_text) > 200 else ''}")
